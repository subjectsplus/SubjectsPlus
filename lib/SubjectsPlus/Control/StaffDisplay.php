...
class StaffDisplay {
	...
	function writeTable( $qualifier, $get_assoc_subs = 1, $print_display = 0 ) {
        ...
        ORDER BY department_sort, d.name, staff_sort DESC, lname";
				...
				$items           = "<ul id=\"uml-staff-departments\" class=\"list-unstyled d-md-flex flex-md-row flex-md-wrap staff-departments\">";
				$current_dept    = "";
				foreach ( $r as $myrow ) {
					...
					if ( $current_dept != $dept_id ) {
						...
						$items .= "<a name=\"$dept_id\"></a><h2 class='uml-all-staff-show' data-dept-id='$dept_id'>$dept_name</h2><div class=\"dept-info d-sm-flex flex-sm-row flex-sm-nowrap\">";

						if ( $dept_tel != "" ) {
							$items .= "<p class='uml-all-staff-show' data-dept-id='$dept_id'><i class=\"fas fa-phone\"></i>$tel_prefix $dept_tel</p>";
						}
					...
					$items .= "<li class='uml-all-staff-show' data-dept-id='$dept_id'>
				<p data-dept-id='$dept_id' class='uml-staff-meta-search' style='display: none;'>
					$lname, $fname, $title, $tel_prefix $tel, $email, $assoc_subjects, $fname $lname
				</p>
				<div class=\"staff-pic\"><a href=\"$link_to_details\">";
					...
					$items .= "</a></div><div class=\"staff-meta\"><h4 class='uml-staff-name'>";
