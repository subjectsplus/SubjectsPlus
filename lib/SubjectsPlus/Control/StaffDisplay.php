...
ORDER BY department_sort, d.name, staff_sort DESC, lname";
                ...
                foreach ($r as $myrow) {
                    ...
                    if ($get_assoc_subs == 1) {
                        ...
                        $assoc_subjects = self::getAssocSubjectsAndLiaisons($staff_id, $ptags);
                    } else {
                        $assoc_subjects = "";
                    }
                    // end subject listing
            ...
            case "Subject Librarians":
                ...
                foreach ($r as $myrow) {
                    ...
                    $sub_query = "select subject, shortform, type from subject, staff_subject
                    WHERE subject.subject_id = staff_subject.subject_id
                    AND staff_id =  '$myrow[5]'
                    AND (type = 'Subject' OR type = 'Liaison')
                    AND active = '1'
        ...
        <thead><tr class=\"staff-heading\"><th><strong>" . _("Librarian") . "</strong></th><th data-hide=\"phone,mid\" data-sort-ignore=\"true\"><strong>" . _("Subject Responsibilities") . "</strong></th></tr></thead>";
            ...
            case "Librarians by Subject Specialty":
                $q = "select lname, fname, title, tel, email, subject, type, staff.staff_id, shortform from
    ...
    function getAssocSubjectsAndLiaisons($staff_id, $ptags)
    {
        global $mod_rewrite;
        $assoc_subjects = "";
        ...
        if ($islib == 1) {
            // UM hack in query
            $q2 = "SELECT subject, shortform, type 
            ...
            foreach ($r2 as $myrow2) {
                ...
                if ($myrow2['type'] == 'Liaison') {
                    $assoc_subjects .= $myrow2[0] . ", ";
                } else {
